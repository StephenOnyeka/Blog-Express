...
import {
  ArrowLeft, Setting2, CloseCircle, Image as ImageIcon,
  TickCircle, Add,
} from 'iconsax-react';
import PageTemplate from '../components/PageTemplate';
import {
  saveArticle, getArticleById, createDraftArticle,
  estimateReadTime, wordCount, CURRENT_USER,
...
const SUGGESTED_TAGS = [
  'Technology', 'AI', 'Design', 'Programming', 'Startups',
  'Science', 'Writing', 'Culture', 'Health', 'Psychology',
...
export default function WritePage({ isLoggedIn, onAuthChange }: WritePageProps) {
  ...
  const canPublish = title.trim().length > 0 && body.replace(/<[^>]+>/g, '').trim().length > 0;
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            ...
            <button
              style={{
                ...
                padding: '10px 24px', fontSize: 15, fontWeight: 500,
                color: 'var(--text-primary)', background: 'none', cursor: 'pointer',
    ...
    <PageTemplate isLoggedIn={isLoggedIn} onAuthChange={onAuthChange} showFooter={false}>
      ...
      <div style={{
        ...
        >
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {saveStatus === 'draft' && <span style={{ color: 'var(--text-muted)' }}>Draft</span>}
          </span>
        </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            ...
            style={{ opacity: canPublish ? 1 : 0.4, transition: 'opacity 0.2s' }}
      ...
      <div style={{ maxWidth: 740, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <div style={{
          ...
          />
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{CURRENT_USER.name}</div>
      ...
      {showPublishPanel && (
        <>
          ...
          <div
            ...
            style={{
              position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 499,
            }}
          />
            ...
            <div className="publish-panel-body">
              ...
              <div className="publish-panel-section">
                <div className="publish-panel-preview">
                  {thumbnail ? (
                    <img src={thumbnail} alt="Cover" className="publish-panel-preview-img" />
                  ) : (
                    <button
                      onClick={() => thumbnailInputRef.current?.click()}
                      style={{
                        width: '100%', height: 160, background: 'var(--bg-tertiary)',
                        border: 'none', cursor: 'pointer', display: 'flex',
                        flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                        gap: 8, color: 'var(--text-muted)', fontFamily: 'var(--font-sans)',
                      }}
                    >
                      <ImageIcon size={28} />
                  ...
                  {thumbnail && (
                    <button
                      ...
                      style={{
                        ...
                        color: 'var(--accent)', padding: '6px', background: 'none',
                  ...
                  <div className="publish-panel-preview-text">
                    <div className="publish-panel-preview-title">{title || 'Story title…'}</div>
                    <div className="publish-panel-preview-subtitle">{subtitle || 'Story subtitle…'}</div>
                  </div>
                </div>
                <p className="publish-panel-hint">
                  Note: Changes here affect how your story appears in public places like BlogNest's homepage — not the story itself.
                </p>
              </div>
              ...
              <div className="publish-panel-section">
                  ...
                  <div className="publish-panel-tags">
                    {tags.map(tag => (
                      <span key={tag} className="publish-panel-tag">
                        {tag}
                        <span className="publish-panel-tag-remove" onClick={() => removeTag(tag)}>×</span>
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div style={{ marginTop: 12 }}>
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>Suggested:</p>
